...
**実装フェーズ（C側アプリ v1・コード完成／実機動作確認は残）**。2026-07-12 に供給側＝データ収集アプリ（`tech/customer/`）の iOS 実装（M0〜M4）を完了。オンボーディング→ミッション→キャプチャ（映像＋IMU・ポカヨケ3段）→保存→貢献記録まで一通り実装し、**ビルド・テスト（94件）・静的解析が全緑、第三者レビューの Blocker ゼロ**。ただし**実機での映像＋IMU 実収録・時刻同期・権限フローは未検証**（本開発環境はシミュレータのみでカメラが無いため）＝「コード完成・シミュレーション検証済み」段階。要件は [`tech/customer/requirements.md`](./tech/customer/requirements.md)、画面は [`tech/customer/screens.md`](./tech/customer/screens.md)、技術選定・全体計画・進捗・最終サマリは [`devlog/2026-07.md`](./devlog/2026-07.md)（追記3以降）。残る事業側の未解決論点は [`biz/roadmap.md`](./biz/roadmap.md)。
